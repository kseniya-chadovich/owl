...
const loadUser = async () => {
    ...
    // If no previous messages → first-time welcome message
    if (!stored.length) {
        messages.value = [
            {
                id: newId(),
    ...
    messages.value = stored.map((t) => {
        // Assistant message with schedules in JSON
        try {
        ...
        if (t.startsWith("User:")) {
            ...
            // NEW FORMAT — pretty printed JSON was stored separately
            if (text === "I confirm this schedule.") {
                // Look for the JSON entry containing the pretty schedule
                const scheduleJson = stored.find((s) => {
                    try {
                        const parsed = JSON.parse(s);
                        return parsed.displaySchedule;
                    } catch {
                        return false;
                    }
                });
                ...
                if (scheduleJson) {
                    ...
                    return {
                        ...
                        displaySchedule: parsed.displaySchedule,
                    };
                }
            ...
            // OLD FORMAT — schedule text included directly under confirm message
            ...
            // Normal user message
            return { id: newId(), role: "user", text };
        }
